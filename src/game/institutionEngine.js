...
import { eventosFederativosExtrasSeed } from '../data/seed/eventosFederativosExtras.js';

const catalogoEventosFederativos=[...eventosFederativosSeed,...eventosFederativosExtrasSeed];
...
export function sortearEventoFederativo({turno=1,estados=[],historico=[],nomeacoes=[],cargos=[],rng=Math.random}){
  ...
  const elegiveis=catalogoEventosFederativos.filter(e=>!usados.has(e.id));
  const pool=elegiveis.length?elegiveis:catalogoEventosFederativos;
